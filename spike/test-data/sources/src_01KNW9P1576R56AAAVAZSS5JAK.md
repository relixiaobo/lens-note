---
id: src_01KNW9P1576R56AAAVAZSS5JAK
type: source
source_type: markdown
title: full-prompt-of-chatgpt-4o-with-canvas
word_count: 1641
raw_file: raw/src_01KNW9P1576R56AAAVAZSS5JAK.md
ingested_at: '2026-04-10T18:15:18.439Z'
created_at: '2026-04-10T18:15:18.439Z'
status: active
---
...
- ChatGPT Canvas (canmore) system prompt
  - complete prompt
...
comments: {
  ...
  - 功能分析
    - 这个prompt定义了一个名为"canmore"的工具,用于创建和更新文本文档,这些文档会在用户对话旁边的"画布"(canvas)上呈现。
    - 主要功能：
      - 为文本文档添加评论（comment_textdoc）
        - 不使用场景:
        - 输出格式
        - 参数:
        - 使用场景:
        - 用途: 为文档添加不属于文档内容的注释
      - 更新文本文档（update_textdoc）
        - 参数:
        - 输出格式
        - 使用场景:
        - 用途: 重写或编辑文档的特定部分
        - 不使用场景:
      - 创建文本文档（create_textdoc）
        - 不使用场景:
        - 用途: 创建新文档或从现有文档派生相关文档
        - 参数:
        - 输出格式
        - 使用场景:
    - 注意事项
      - 对于文本文档,仅在用户要求更改特定、小范围部分时才进行局部更新
      - 更新文档时,通常应重写整个文档,特别是对于代码文档
      - 对于图表、表格或执行Python代码,应使用python工具而非canmore
      - 代码内容应使用code/*类型,并使用create_textdoc
      - 倾向于不使用create_textdoc,除非内容确实需要单独展示
      - 可以在一条消息中创建多个内容,但尽量每条消息只创建一个
