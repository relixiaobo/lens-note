...
function createTestNote(title: string): string {
  ...
  try {
    ...
    return JSON.parse(stdout).data.id;
  } finally {
    unlinkSync(tmpFile);
  }
}
...
describe("lens index (Schlagwortregister)", () => {
  ...
  it("removes single entry from keyword", () => {
    ...
    const show = JSON.parse(listOut).data;
  ...
  it("works via --stdin dispatch", () => {
    ...
    const addOut = JSON.parse(addResult.stdout).data;
    ...
    const showOut = JSON.parse(showResult.stdout).data;
    assert.equal(showOut.keyword, "StdinKW");

    // List
    const listResult = lensStdin({ command: "index" });
    assert.equal(listResult.exitCode, 0);
    const listOut = JSON.parse(listResult.stdout).data;
    ...
    const removeOut = JSON.parse(removeResult.stdout).data;
