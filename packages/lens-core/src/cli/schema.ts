...
const COMMANDS: Record<string, CommandSpec> = {
  ...
  board: {
    description: "Whiteboard workspace commands. Whiteboards are the primary workspace layer — they reference graph objects as members but do NOT create graph edges. Arrows drawn on a whiteboard are local free-text labels, NOT typed rels; use arrow-promote to convert one to a graph rel. See docs/whiteboard-model.md. Subcommands: create list show add remove move layout update delete find camera group ungroup arrow arrow-remove arrow-promote.",
    readonly: false,
    positional: [
      { name: "subcommand", type: "string", description: "One of: create, list, show, add, remove, move, layout, update, delete, find, camera, group, ungroup, arrow, arrow-remove, arrow-promote", required: true },
      { name: "args", type: "string", description: "Subcommand arguments (wb-id, card-id, grp-id, arr-id as needed)", variadic: true },
    ],
    flags: {
      title: { type: "string", description: "Title for create/update" },
      body: { type: "string", description: "Body (research log) for create/update" },
      positions: { type: "string", description: "JSON map of card_id → {x, y} for layout" },
      x: { type: "number", description: "X coordinate (for move/camera/group)" },
      y: { type: "number", description: "Y coordinate" },
      scale: { type: "number", description: "Camera zoom scale (camera subcommand)" },
      width: { type: "number", description: "Width (group)" },
      height: { type: "number", description: "Height (group)" },
      parent: { type: "string", description: "Group ID to nest a moved member under (grp_<ULID>)" },
      "no-parent": { type: "boolean", description: "Clear the parent assignment when moving a member" },
      label: { type: "string", description: "Group label, arrow free-text label" },
      color: { type: "string", description: "Optional color hint (group/arrow)" },
      members: { type: "string", description: "Comma-separated card IDs to place into a new group" },
      from: { type: "string", description: "Arrow source member ID" },
      to: { type: "string", description: "Arrow target member ID" },
      style: { type: "string", description: "Arrow line style: solid | dashed" },
      rel: { type: "string", description: "Graph rel for arrow-promote: supports | contradicts | refines | related | continues" },
      reason: { type: "string", description: "Reason text for promoted graph rel" },
    },
    output: "Each subcommand returns a JSON envelope. create/show/add/remove/move/layout/update return the updated whiteboard state. list returns {count, items}. find returns {card_id, count, items}. delete returns {id, deleted:true}. group/arrow return the new child object. arrow-promote returns the arrow plus the created graph link.",
    examples: [
      ...
      { description: "Move a single card", request: { command: "board", positional: ["move", "wb_01ABC...", "note_01X..."], flags: { x: 120, y: -40 } } },
      { description: "Set the default camera", request: { command: "board", positional: ["camera", "wb_01ABC..."], flags: { x: 0, y: 0, scale: 1 } } },
      { description: "Create a group with two members", request: { command: "board", positional: ["group", "wb_01ABC..."], flags: { label: "Evidence", x: 0, y: 0, width: 800, height: 400, members: "note_01X...,note_01Y..." } } },
      { description: "Draw an arrow between two cards", request: { command: "board", positional: ["arrow", "wb_01ABC..."], flags: { from: "note_01X...", to: "note_01Y...", label: "rebuts" } } },
      { description: "Promote an arrow to a graph rel", request: { command: "board", positional: ["arrow-promote", "wb_01ABC...", "arr_01..."], flags: { rel: "contradicts", reason: "Reviewed and confirmed" } } },
